...
def _blocking_update_external_db(chat_id, chat_title, amount_to_add):
    """
    تابع داخلی بلاکینگ برای آپدیت دیتابیس حسابداری سراب.
    این تابع در یک ترد جداگانه اجرا می‌شود.
    """
    try:
        # اگر مبلغ صفر است، عملیاتی انجام نده
        if amount_to_add == 0:
            logging.info(f"Skipping external DB update for {chat_id}, amount is zero.")
            return True # از نظر فنی موفقیت‌آمیز است، کاری برای انجام نبوده
            
        with sqlite3.connect(EXTERNAL_DB_PATH) as conn:
            cursor = conn.cursor()
            
            # ۱. اطمینان از وجود گروه، تنظیم عنوان و فعال‌سازی
            # (اگر گروه وجود داشته باشد، فقط عنوان و وضعیت فعال بودن آپدیت می‌شود)
            cursor.execute('''
        ...
        # خطاهایی مثل "no such table" یا "database is locked"
...
async def update_external_db_balance(chat_id, chat_title, amount_to_add):
    """
    رابط (wrapper) ناهمزمان برای آپدیت دیتابیس خارجی 'hesabdar'
    در یک ترد جداگانه تا از بلاک شدن برنامه جلوگیری شود.
    """
...
# --- ⭐️⭐️ (اصلاح شده) تابع گزارش‌دهی خودکار نیمه‌شب ⭐️⭐️ ---
...
async def send_daily_reports():
    ...
    # --- ۱. ارسال گزارش به گروه‌ها و آپدیت دیتابیس خارجی ---
    ...
    for chat_id in active_groups_yesterday:
        ...
        try:
            ...
            # --- ⭐️⭐️ شروع بخش جدید ⭐️⭐️ ---
            
            # ۱. محاسبه جمع کل به دلار
            total_usdt = Decimal('0')
            if tether_price > 0:
                total_usdt = total_toman / tether_price
            
            # ۲. آپدیت دیتابیس خارجی (حساب سراب)
            int_total_toman = int(total_toman)
            formatted_toman = f"{int_total_toman:,.0f}"
            confirmation_msg = "" # پیام تاییدی برای انتهای گزارش
            
            try:
                # ارسال عنوان گروه برای ثبت یا آپدیت در دیتابیس خارجی
                ...
                if update_success:
                    ...
                    # اطلاع‌رسانی به مالک در مورد خطای دیتابیس خارجی
                    await send_long_message(OWNER_ID, f"🚨 **هشدار عدم بروزرسانی حساب سراب** 🚨\n\n"
                                                      f"گروه: {chat_title} (`{chat_id}`)\n"
                                                      f"مبلغ: `{formatted_toman}` تومان\n"
                                                      f"خطا در اتصال یا آپدیت دیتابیس `{EXTERNAL_DB_PATH}` رخ داد. لطفاً لاگ‌ها را بررسی کنید.")
            except Exception as e:
                logging.error(f"Critical error during external DB update call: {e}")
                confirmation_msg = f"\n⚠️ **خطای سیستمی:** در پردازش حساب سراب مشکلی پیش آمد."
                # اطلاع‌رسانی به مالک در مورد خطای بحرانی
                await send_long_message(OWNER_ID, f"🚨 **خطای بحرانی در آپدیت حساب سراب** 🚨\n"
                                                  f"گروه: {chat_title} (`{chat_id}`)\n"
                                                  f"خطا: {e}")

            # ۳. ساخت پیام گزارش گروه
            msg = f"📊 **خلاصه آمار روزانه** 📊\n"
            msg += f"🗓 **تاریخ:** `{yesterday_date}`\n"
            msg += f"🏠 **گروه:** {chat_title}\n\n"
            msg += f"💰 **جمع کل فروش (تومان):** `{total_toman:,.0f}`\n"
            # (جدید) افزودن جمع کل دلار
            ...
            # (جدید) افزودن پیام تایید دیتابیس خارجی
            msg += confirmation_msg
            
            # --- ⭐️⭐️ پایان بخش جدید ⭐️⭐️ ---
    ...
    # --- ۲. ارسال گزارش به پی‌وی مالک (بدون تغییر) ---
