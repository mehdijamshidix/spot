...
class Database:
    ...
    # --- ⭐️ توابع گزارش‌گیری جدید برای پرداخت‌ها ⭐️ ---
...
# --- ⭐️⭐️ (کاملاً جدید) توابع دیتابیس خارجی برای ادمین‌ها ⭐️⭐️ ---

def _blocking_update_admin_balance(user_id, username, amount_to_add, reason):
    try:
        if amount_to_add == 0:
            logging.info(f"Skipping external admin DB update for {user_id}, amount is zero.")
            return True
        ...
        with sqlite3.connect(EXTERNAL_DB_PATH) as conn:
            ...
            ''')
            cursor.execute('''
            INSERT INTO admins (user_id, username, balance)
            VALUES (?, ?, 0)
            ON CONFLICT(user_id) DO UPDATE SET
            username = COALESCE(excluded.username, admins.username)
            ''', (user_id, username))
            cursor.execute('''
            UPDATE admins SET balance = balance + ?
            WHERE user_id = ?
            ''', (amount_to_add, user_id))
            transaction_date = datetime.now(TEHRAN_TZ).strftime('%Y-%m-%d %H:%M:%S')
            cursor.execute('''
            INSERT INTO admin_transactions (user_id, date, amount, reason)
            VALUES (?, ?, ?, ?)
            ''', (user_id, transaction_date, amount_to_add, reason))
            conn.commit()
            logging.info(f"Successfully updated external admin balance for {user_id}. Added {amount_to_add} for {reason}.")
            return True
    except sqlite3.OperationalError as e:
        logging.error(f"External Admin DB OperationalError for {user_id} at {EXTERNAL_DB_PATH}: {e}")
        return False
    except Exception as e:
        logging.error(f"Failed to update external admin balance for {user_id} at {EXTERNAL_DB_PATH}: {e}")
        return False

async def update_external_admin_balance(user_id, username, amount_to_add, reason):
    amount_int = int(amount_to_add)
    if amount_int <= 0:
        return True
    return await asyncio.to_thread(_blocking_update_admin_balance, user_id, username, amount_int, reason)
...
async def get_user_from_event(event):
    ...
    # ⭐️ اصلاح شد: فقط اولین آرگومان را به عنوان کاربر در نظر می‌گیرد
    # این برای /addspecial @user 500 ضروری است
...
async def help_handler(event):
    ...
    if sender_id == OWNER_ID:
        help_text += (
            ...
            "`/addspecial <ID/@> <rate>` (افزودن/تغییر نرخ کمیسیون)\n"
        ...
        async with client.conversation(event.chat_id, timeout=180) as conv:
            ...
            try:
                ...
                await conv.send_message(
                    ...
                    "💬 اکنون، لطفاً **نرخ کمیسیون** فردی او برای هر S (Count) *کل* را به تومان وارد کنید.\n"
                    "مثال: `1000`\n\n(برای لغو /cancel را ارسال کنید.)"
                )
                ...
                await conv.send_message(
                    ...
                    f"نرخ کمیسیون: **{rate:,.0f} تومان** به ازای هر S (Count) *کل*\n"
...
async def list_special_admins_callback(event):
    if event.sender_id != OWNER_ID:
        await event.answer("شما مجاز به استفاده از این دکمه نیستید.", alert=True)
        return
    await list_special_admins_helper(event, special_admins_cache, "لیست ادمین‌های ویژه", b"panel_special_admins")
...
async def list_special_admins_handler(event):
    ...
    msg = "**لیست ادمین‌های ویژه (کمیسیون S کل):**\n\n"
...
async def status_handler(event):
    ...
    try:
        ...
        msg += "\n--- **ادمین‌های ویژه (با نرخ کمیسیون فردی)** ---\n"
        if not special_admins_cache:
...
async def stats_handler(event):
    """(اصلاح شده) نمایش آمار لحظه‌ای *امروز* گروه"""
    if not event.is_group:
        await event.reply("❌ این دستور فقط باید در گروه استفاده شود.")
        return
    ...
    msg += "--- 👨‍💻 آمار ادمین‌ها (دستمزد S Sum) ---\n"
    ...
    if not admin_stats:
        ...
        user_map = {admin_id: (result.first_name if isinstance(result, User) else f"(کاربر {admin_id})") for admin_id, result in zip(admin_ids, user_results)}
            
        admin_stats_sorted = sorted(admin_stats, key=lambda x: x[1], reverse=True)
...
async def send_daily_reports():
    """
    در ساعت 00:01 اجرا می‌شود، آمار روز گذشته را جمع‌آوری، در دیتابیس خارجی ثبت و ارسال می‌کند.
    """
    ...
    e_point_value = Decimal(db.get_setting('e_point_value', '0')) # دستمزد E-Point
    ...
    # --- ⭐️⭐️ ۳. (منطق جدید) محاسبه و پرداخت ادمین‌ها و ارسال نوتیفیکیشن ⭐️⭐️ ---
    logging.info("Starting admin payout calculations...")
    
    # دریافت آمار مورد نیاز
    e_point_stats = db.get_all_admin_e_stats(yesterday_date) # {id: points}
    s_count_stats = db.get_all_admin_s_counts(yesterday_date) # {id: count}
    all_admin_salary_stats = db.get_all_admin_salary_stats(yesterday_date) # [(id, s_sum)]
    s_sum_map = dict(all_admin_salary_stats) # تبدیل به دیکشنری {id: s_sum}
    
    # دریافت ادمین‌های ویژه و نرخ‌های کمیسیونشان
    special_admin_rates = special_admins_cache # {id: rate}
    
    # محاسبه مجموع کل S Count (برای کمیسیون)
    total_s_count_all_admins = sum(s_count_stats.values())
    logging.info(f"Total S-Count for all admins (for commission): {total_s_count_all_admins}")
    
    # جمع‌آوری همه ادمین‌ها (عادی و ویژه)
    all_admin_ids = bot_admins_cache # از کش لود شده در استارت
    
    if not all_admin_ids:
        logging.info("No admins found to process payouts for.")
    else:
        # دریافت اطلاعات کاربران برای نام و یوزرنیم
        tasks = [client.get_entity(admin_id) for admin_id in all_admin_ids]
        user_results = await asyncio.gather(*tasks, return_exceptions=True)
        user_map = {admin_id: result for admin_id, result in zip(all_admin_ids, user_results) if isinstance(result, User)}

        for admin_id in all_admin_ids:
            user_entity = user_map.get(admin_id)
            if not user_entity:
                logging.warning(f"Skipping payout for unfetchable user {admin_id}")
                continue
                
            admin_username = user_entity.username
            admin_firstname = user_entity.first_name
            
            total_admin_payout = Decimal('0')
            payout_details_for_notification = []
            
            # --- الف) محاسبه پرداخت S-Sum (برای همه ادمین‌ها، شامل ویژه‌ها) ---
            s_sum = Decimal(s_sum_map.get(admin_id, 0))
            if s_sum > 0 and salary_rate > 0:
                s_sum_payout = s_sum * salary_rate
                total_admin_payout += s_sum_payout
                reason = f"بابت {s_sum} مجموع S (Sum) شخصی در تاریخ {yesterday_date}"
                payout_details_for_notification.append(f"• بابت {s_sum} S (Sum) شخصی: `{s_sum_payout:,.0f}` تومان")
                # ⭐️ واریز به حسابدار
                await update_external_admin_balance(admin_id, admin_username, s_sum_payout, reason)

            # --- ب) محاسبه پرداخت امتیاز E (برای همه ادمین‌ها، شامل ویژه‌ها) ---
            e_points = e_point_stats.get(admin_id, 0)
            if e_points > 0 and e_point_value > 0:
                e_payout = e_points * e_point_value
                total_admin_payout += e_payout
                reason = f"بابت {e_points} امتیاز E در تاریخ {yesterday_date}"
                payout_details_for_notification.append(f"• بابت {e_points} امتیاز E: `{e_payout:,.0f}` تومان")
                # ⭐️ واریز به حسابدار
                await update_external_admin_balance(admin_id, admin_username, e_payout, reason)

            # --- ج) محاسبه پرداخت کمیسیون S-Count (فقط برای ادمین‌های ویژه) ---
            if admin_id in special_admin_rates:
                override_rate = special_admin_rates[admin_id] # دریافت نرخ کمیسیون فردی
                if total_s_count_all_admins > 0 and override_rate > 0:
                    override_payout = total_s_count_all_admins * override_rate
                    total_admin_payout += override_payout
                    reason = f"بابت کمیسیون {total_s_count_all_admins} S کل (نرخ {override_rate:,.0f} T) در تاریخ {yesterday_date}"
                    payout_details_for_notification.append(
                        f"⭐️ **بابت کمیسیون ویژه:** `{override_payout:,.0f}` تومان\n"
                        f"   (تعداد {total_s_count_all_admins} S کل × نرخ {override_rate:,.0f} تومان)"
                    )
                    # ⭐️ واریز به حسابدار
                    await update_external_admin_balance(admin_id, admin_username, override_payout, reason)

            # --- د) ارسال نوتیفیکیشن به ادمین (فقط اگر درآمدی داشته) ---
            if total_admin_payout > 0:
                try:
                    notification_msg = f"📣 **گزارش واریز به حساب**\nسلام {admin_firstname}!\n\n"
                    notification_msg += f"مبالغ زیر (مربوط به تاریخ `{yesterday_date}`) به حساب شما در سامانه حسابداری اضافه شد:\n\n"
                    notification_msg += "\n\n".join(payout_details_for_notification)
                    notification_msg += f"\n\n\n**مجموع واریزی امشب:** `{total_admin_payout:,.0f}` **تومان**"
                    
                    await send_long_message(admin_id, notification_msg)
                except Exception as e:
                    logging.error(f"Failed to send payout notification to admin {admin_id}: {e}")
                    await send_long_message(OWNER_ID, f"⚠️ **خطا در ارسال نوتیفیکیشن پرداخت**\n"
                                                      f"عدم موفقیت در ارسال پیام به {admin_firstname} (`{admin_id}`).\n"
                                                      f"مبلغ: `{total_admin_payout:,.0f}` تومان\n"
                                                      f"خطا: {e}")


    # --- ۴. ارسال گزارش‌های خلاصه به پی‌وی مالک ---
    
    # گزارش الف: آمار دستمزد (S Sum - مثل قبل، برای اطلاع مالک)
    ...
    # (all_admin_salary_stats از بالا موجود است)
    ...
    group_stats_msg += f"(توجه: این سود خالص، پرداخت‌های E و کمیسیون‌های ویژه به ادمین‌ها را کسر نکرده است)"
